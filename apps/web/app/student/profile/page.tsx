      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="border border-white/15 bg-[#08090c]/80 backdrop-blur-md p-6 rounded-none shadow-xl text-center relative flex flex-col items-center">
            ...
            <p className="text-xs text-emerald-300 mt-0.5">Undergraduate Student</p>
            <p className="text-[11px] text-white/50 mt-0.5">Matric ID: {profile.institutional_id}</p>

            <div className="border-t border-white/10 mt-5 pt-5 w-full space-y-3 text-left text-xs relative">
              <div className="absolute -top-3 right-0">
                {!isEditing ? (
                  <button 
                    onClick={() => setIsEditing(true)}
                    className="border border-white/15 bg-white/5 hover:bg-white/10 text-white/70 hover:text-white p-1 rounded-none transition-colors cursor-pointer text-[10px] uppercase px-2 font-medium"
                  ...
                  <div className="flex items-center gap-1">
                    <button 
                      ...
                      className="border border-white/15 bg-white/5 hover:bg-red-500/20 text-red-300 p-1 rounded-none transition-colors cursor-pointer text-[10px] px-2 font-medium"
                    >
                      Cancel
                    </button>
                    <button 
                      onClick={handleSave}
                      disabled={isSaving}
                      className="border border-emerald-400/40 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 p-1 rounded-none transition-colors cursor-pointer text-[10px] px-2 disabled:opacity-50 font-medium"
                    >
                      Save
                    </button>
                  </div>
              ...
              {isEditing ? (
                <div className="space-y-2 pt-2">
                  <div className="space-y-1">
                    <label className="text-[10px] uppercase text-white/60 font-medium">Phone Number</label>
                    <div className="flex items-center gap-2 border border-white/20 bg-black/40 px-3 py-1.5 rounded-none">
                      ...
                      <input 
                        ...
                        className="bg-transparent border-none text-xs w-full focus:outline-none text-white"
                        placeholder="+60 12-345 6789"
                      />
                    </div>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] uppercase text-white/60 font-medium">Emergency Contact</label>
                    <div className="flex items-center gap-2 border border-white/20 bg-black/40 px-3 py-1.5 rounded-none">
            ...
            <div className="w-full border-t border-white/10 mt-5 pt-4">
              <div className="relative group/disabled w-full cursor-not-allowed" title="Disabled Feature">
                <div className="w-full flex items-center justify-center gap-2 border border-white/10 bg-white/5 text-white/40 py-2.5 px-3 text-xs uppercase tracking-wider pointer-events-none select-none rounded-none font-medium">
                  <PixelIcon name="award" size={14} />
                  Merit Claims [ DISABLED ]
                </div>
                <div className="pointer-events-none absolute -top-8 left-1/2 -translate-x-1/2 hidden group-hover/disabled:flex items-center px-2 py-1 text-[11px] text-white bg-black border border-white/20 whitespace-nowrap z-50 font-bold">
                  Disabled Feature
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {enrolledCourses.length > 0 ? (
                enrolledCourses.map((course, idx) => (
                  <div
                    key={idx}
                    className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3.5 border border-white/10 bg-white/5 hover:border-emerald-400/40 rounded-none transition-colors gap-2"
                  >
                    <div>
                      ...
                      <h5 className="font-bold text-white text-sm mt-1.5">{course.name}</h5>
                    </div>
                ...
                <p className="text-xs text-white/40 py-6 text-center">No enrolled courses found.</p>
              )}
            </div>
          </div>
        </div>
