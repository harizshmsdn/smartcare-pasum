...
const hexToRgb = (hex: string): [number, number, number] => {
  ...
  // Support 3-character shorthand syntax (e.g. #fff -> #ffffff)
...
const Grainient: React.FC<GrainientProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("WebGL 2 context creation failed, falling back to static gradient:", e);
      setHasWebGL(false);
      return;
    }
    ...
    const io = new IntersectionObserver(
      ([entry]) => { isVisible = entry ? entry.isIntersecting : true; isVisible ? tryStart() : tryStop(); },
      { threshold: 0 }
    );
  ...
  useEffect(() => {
    ...
    u.uZoom.value           = zoom;
    u.uColor1.value         = new Float32Array(hexToRgb(color1 ?? '#FF9FFC'));
    u.uColor2.value         = new Float32Array(hexToRgb(color2 ?? '#5227FF'));
    u.uColor3.value         = new Float32Array(hexToRgb(color3 ?? '#B497CF'));
  }, [
    timeSpeed, colorBalance, warpStrength, warpFrequency, warpSpeed,
    warpAmplitude, blendAngle, blendSoftness, rotationAmount, noiseScale,
    grainAmount, grainScale, grainAnimated, contrast, gamma, saturation,
    centerX, centerY, zoom, color1, color2, color3, hasWebGL
  ]);
  ...
  if (!hasWebGL) {
    ...
    return <div style={fallbackStyle} className={`h-full w-full ${className}`} />;
  }
